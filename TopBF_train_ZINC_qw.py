...
import os, sys
os.environ["CUDA_VISIBLE_DEVICES"] = "1"  # 只使用第1张卡
import json
import pickle
...
from core.model.bfn.bfn_base_ZINC_qw_fast import bfn4GraphTransformer # Our new GraphTransformer-based model
import torch
import os
...
class BFN4GraphGenTrain(pl.LightningModule): # Renamed class for clarity
    def __init__(self, config, node_feats, edge_feats, max_nodes):
        ...
        qw_kwargs = {
            ...
            'qw_dist_cap': self.cfg.dynamics.qw_dist_cap,
            'qw_on_edges': self.cfg.dynamics.qw_on_edges,
            'qw_edge_max_nodes': self.cfg.dynamics.qw_edge_max_nodes,
            'qw_edge_max_pairs': self.cfg.dynamics.qw_edge_max_pairs
        }
        ...
        self.dynamics = bfn4GraphTransformer(
            ...
            bins_X=node_feats,
            bins_A=edge_feats,
            scale_X=self.cfg.dynamics.scale_X,
            scale_A=self.cfg.dynamics.scale_A,
            **qw_kwargs
        )
        # pdb.set_trace()
...
def main():
    ...
    parser.add_argument("--config_file", type=str, default="configs/ZINC_qw.yaml") # Assuming a config file structure
    ...
    # parser.add_argument("--sigma1_X", type=float, default=0.5)
    ...
    # parser.add_argument("--ckpt_pattern", type=str, default="epoch=489-val_loss=0.7388114333152771.ckpt")
    ...
    print(f"scale_X: {cfg.dynamics.scale_X}\tscale_A: {cfg.dynamics.scale_A}")
    print(f"sigma1_X: {cfg.dynamics.sigma1_X}\tsigma1_A: {cfg.dynamics.sigma1_A}")
    ...
    callbacks = [
        ...
        EMACallback(decay=0.9999, ema_device="cpu"),
        ModelCheckpoint(
    ...
    if not cfg.test:
        ...
        x_1hot = torch.nn.functional.one_hot(x_idx, num_classes=9).float() * node_mask.unsqueeze(-1)

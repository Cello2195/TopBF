...
\usepackage{multicol}
\usepackage{pifont} % 导入 pifont 宏包
\usepackage{bm}
...
\title[Short Article Title]{Transport-Coupled Bayesian Flows for Molecular Graph Generation}

\author[1]{Yida Xiong\ORCID{0009-0003-3182-3659}}
\author[1]{Jiameng Chen\ORCID{0009-0004-3414-3621}}
\author[1]{Kun Li\ORCID{0000-0002-7002-234X}}
\author[1]{Hongzhi Zhang\ORCID{0009-0002-3726-5056}}
\author[1]{Xiantao Cai}
\author[2]{Jia Wu}
\author[3,$\ast$]{Lei Lei}
\author[1,4,$\ast$]{Wenbin Hu\ORCID{0000-0002-9258-3850}}
...
\address[2]{\orgdiv{Department of Computing}, \orgname{Macquarie University}, \orgaddress{\street{Sydney}, \country{Australia}}}
\address[3]{\orgdiv{Institute of Information on Traditional Chinese Medicine}, \orgname{China Academy of Chinese Medical Sciences}, \orgaddress{\country{China}}}
\address[4]{\orgdiv{Wuhan University Shenzhen Research Institute}, \orgaddress{\street{Shenzhen}, \country{China}}}
% $^3$Wuhan University Shenzhen Research Institute, Shenzhen, China\\
...
% \keywords{Molecular Graph Generation, Bayesian Flow Network, Optimal-Transport Coupling, Drug Discovery}
...
% discrete (state) diffusion models for continuous/discrete data，用法统一一下
% class 还是 category？？？？？？？？？？

Molecular graph generation (MGG) is a critical yet challenging endeavor in modern computational drug discovery (\citealt{li2025graph}), facilitating the advancement of downstream tasks such as molecular optimization (\citealt{zhu2023sample,XIONG2026103907}) and drug–target binding affinity prediction (\citealt{wu2024psc,li2025contrastive}). Early computational approaches utilizing deep learning attempted to generate molecular graphs in a one-shot manner (\citealt{zang2020moflow,kong2022molecule}). However, these methods often struggle to model complex structures effectively. In recent years, diffusion models have demonstrated remarkable performance across various generation tasks (\citealt{gong2024text,chen2025antibody}). These models are generally divided into discrete and continuous types and have been progressively integrated into molecular graph generation.
...
% 这一段写得没有前后逻辑
Recently, Bayesian Flow Networks (BFNs) (\citealt{graves2023bayesian,song2024unified}) provide an alternative paradigm for discrete generation by evolving distribution parameters through a continuous-time Bayesian updating process. Instead of directly predicting discrete values, BFNs propagate a parameter state that represents uncertainty and update it via pseudo-observations, yielding a continuous and differentiable learning process for discrete data. 
% However, due to all categories being equidistant within the probability simplex, the model cannot progressively approach objectives in a manner akin to that in continuous space (\citealt{graves2023bayesian}). As a consequence, this results in a convergence process towards data that appears slower and noisier. 
...
Deep generative models for MGG are typically categorized into autoregressive and one-shot models. The former generate graphs incrementally, adding atoms and bonds in an autoregressive manner (\citealt{you2018graph,you2018graphrnn,jin2018junction,jin2020hierarchical}). 
The latter generate the entire graph at once (\citealt{simonovsky2018graphvae,shi2020graphaf,luo2021graphdf,martinkus2022spectre}).

% \subsubsection{Diffusion Models for Graph.} 
Diffusion models have been adapted for molecular graph generation (\citealt{liu2023generative,qiu2023reconstructing,li2024regressor}). 
...
Some methods define a diffusion process directly on the discrete state space (\citealt{vignac2023digress,xu2024discrete}), which can be complex to accomplish. Others map discrete attributes to a continuous space and apply standard Gaussian diffusion (\citealt{huang2022graphgdp,lee2023exploring}).
...
\Pi(\mathbf{a},\mathbf{b})=\{P\ge 0\mid P\mathbf{1}=\mathbf{a},~P^\top\mathbf{1}=\mathbf{b}\}$. In practice, we solve the entropy-regularized OT and obtain the coupling $P$ via Sinkhorn iterations (\citealt{cuturi2013sinkhorn}).

Quasi-Wasserstein (QW) extends this idea from comparing single scalar signals on atoms to comparing multi-class label fields on graphs. Specifically, for a categorical field with $K$ classes, we view the category probability map $P(\cdot,k)\in[0,1]^N$ as a mass distribution over atom indices for each class $k$, and compare prediction $P(\cdot,k)$ and target $Q(\cdot,k)$ by transporting mass within each class under the same geodesic cost $C$. The QW discrepancy then aggregates these per-class transport costs: 
$
...
% We choose QM9 (\citealt{ramakrishnan2014quantum}) and ZINC250k (\citealt{irwin2012zinc}) for molecular graph generation.

We validate our approach on the QM9 (\citealt{ramakrishnan2014quantum}) and ZINC250k (\citealt{irwin2012zinc}) benchmarks, following the previous experimental setup (\citealt{jo2022score}). QM9 contains approximately 133,885 molecules with up to 9 heavy atoms of 4 types, while ZINC250k comprises roughly 249,455 molecules with up to 38 heavy atoms of 9 types, both covering three standard bond types. Adhering to the standard pipeline (\citealt{shi2020graphaf,luo2021graphdf,jo2022score}), molecules undergo kekulization via RDKit, with explicit hydrogens excluded to focus on the heavy-atom graph representation.

We evaluate the experiments following previous evaluation setting (\citealt{jo2022score}): Validity without correction and uniqueness on 10,000 generated molecules. Fréchet ChemNet Distance (FCD) (\citealt{preuer2018frechet}) measures the distance between training and generated sets using the activations from the penultimate layer of the ChemNet. Additionally, Neighborhood Subgraph Pairwise Distance Kernel (NSPDK) maximum mean discrepancy (MMD) (\citealt{costa2010fast}) evaluates the MMD between the generated molecules and test molecules which takes into account both the atom and bond features for evaluation.
...
Rather than explicitly simulating all intermediate steps, there exists a \emph{Bayesian flow distribution} $p_F(\boldsymbol{\theta}\,|\,\mathbf{G}; t)$ describing the distribution of input parameters at time $t$ after aggregating all past Bayesian updates, as elaborated in Sec.~\ref{pre_BFN}.
...
We make topology awareness a first-class objective by minimizing the transport work needed to align predicted categorical mass with the ground truth (\citealt{peyre2016gromov}).
...
\begin{table}
  \centering
  \caption{Comparison on mean absolute error (MAE) between unconditional generation and PEG guidance generation.}
  \renewcommand \arraystretch{1.}
...
TopBF supports conditional generation without retraining the unconditional flow model. Integrating property control via classifier-free (\citealt{ho2021classifier}) or regressor-free (\citealt{li2024regressor}) guidance often necessitates retraining models from scratch for each target property, which limits their flexibility across different tasks. Instead, we train a separate regressor and apply property-error gradient (PEG) guidance during sampling, which remains flexible for changing targets or composing multiple objectives.
...
\begin{table*}
  \centering
  \caption{Ablation study on Quasi-Wasserstein transport coupling. The best results are highlighted in \textbf{bold} and the second best are \underline{underlined}.}
  \renewcommand \arraystretch{1.}
...
EDP-GNN (\citealt{niu2020permutation}) models graphs with a permutation invariant approach. 
GDSS (\citealt{jo2022score}) is a score-based diffusion model. 
DiGress (\citealt{vignac2023digress}) generates graphs using a discrete diffusion process. 
LGD (\citealt{zhou2024unifying}) provides a unified framework for all-level graph generation and prediction via latent diffusion.
GruM (\citealt{jo2024graph}) models the generative process as a mixture of endpoint-conditioned diffusion.
DeFoG (\citealt{qinmadeira2024defog}) using a discrete flow matching framework that defines continuous-time probability paths over categorical attributes.
SID (\citealt{boget2025simple}) assumes conditional independence between intermediate states to simplify discrete diffusion.
...
Table~\ref{tab:1} summarizes the main results on QM9 and ZINC250k. Overall, TopBF performs strongly on both datasets and achieves the best or tied-best results on most metrics while using only $200$ sampling steps. This is noteworthy because the main competing diffusion and flow-based methods typically require $500$ or $1000$ steps. The results therefore suggest that TopBF improves sample quality without relying on long sampling trajectories.

On QM9, TopBF attains the best validity ($99.74\%$), the best uniqueness ($99.27$), and the best FCD ($0.093$), while remaining tied for second on NSPDK ($0.0002$). Although the absolute improvement in FCD over the strongest baselines is not large, the comparison is already in a near-saturated regime on QM9, where further gains are difficult to obtain. In this setting, the simultaneous improvement in FCD and uniqueness is particularly meaningful, as it indicates that TopBF does not improve distribution matching by sacrificing diversity. Instead, it produces valid and diverse molecules whose overall distribution remains closer to the reference set.

On ZINC250k, which is more challenging because of larger graphs and a richer atom vocabulary, TopBF shows a favorable overall profile. It achieves perfect uniqueness at the reported precision, the best FCD ($1.392$), and the best-tied NSPDK ($0.0008$), while maintaining high validity ($99.37\%$). Compared with DeFoG, TopBF further improves FCD from $1.425$ to $1.392$ with substantially fewer sampling steps. Compared with SID, TopBF is slightly lower in raw validity ($99.37\%$ versus $99.50\%$) but yields a markedly better FCD ($1.392$ versus $2.010$), suggesting a better match to the global chemical distribution. Taken together, these results indicate that TopBF offers a balanced improvement in validity, diversity, and distributional fidelity across both easy and difficult benchmarks. Randomly selected generated molecules are provided in Appendix 7.



\subsection{Conditional Generation}

% We assess the controllability of TopBF on QM9 by conditioning on graph-level properties, following the setting of DiGress (\citealt{vignac2023digress}). From the QM9 test split we randomly select $100$ molecules, record their dipole moment $\mu$ and HOMO energy, and use each pair $(\mu^\star,\mathrm{HOMO}^\star)$ as a target. For every target we generate $10$ molecules unconditionally and conditionally with PEG guidance, respectively. $\mu$ and HOMO are computed with Psi4 (\citealt{smith2020psi4}) from conformers built with RDKit \footnote{\url{https://www.rdkit.org}}, and mean absolute error (MAE) is reported between the targets and the estimated properties for above three tasks. We also carry out sensitivity analysis on $\lambda$. As shown in Table~\ref{tab:qm9-conditional}, TopBF with PEG guidance consistently yields lower MAE than the unconditional sampler, demonstrating effective property control. PEG with $\lambda=1.0$ performs best, as we infer that $\lambda=0.5$ guides little during sampling and guidance strength with $\lambda=1.5$ invades the original gradient to generate valid molecules, affecting the property values conversely.
...
Supplementary data are available as an attachment.

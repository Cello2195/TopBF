...
def get_transform_fn(dataset, adj_scale=3.0):
    if dataset == 'QM9':
        def transform(data):
            x, adj, labels = data
            # pdb.set_trace()
    ...
    elif dataset == 'ZINC250k' or dataset == 'ZINC250k_mini':
        def transform(data):
            # pdb.set_trace()
            x, adj, labels = data
            # the last place is for virtual nodes
...
def dataloader(config, get_graph_list=False):
    ...
    mols = load_mol(os.path.join(config.dataset.datadir, f'{config.dataset.name.lower()}_kekulized.npz'))
    # pdb.set_trace()

    with open(os.path.join(config.dataset.datadir, f'valid_idx_{config.dataset.name.lower()}.json')) as f:
    ...
    # pdb.set_trace()
    train_dataset = MolDataset(train_mols, get_transform_fn(config.dataset.name, adj_scale=config.dynamics.adj_scale))
    test_dataset = MolDataset(test_mols, get_transform_fn(config.dataset.name, adj_scale=config.dynamics.adj_scale))
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=config.optimization.batch_size, shuffle=True, num_workers=config.dataset.num_workers)
    test_dataloader = DataLoader(test_dataset, batch_size=config.optimization.batch_size, shuffle=False, num_workers=config.dataset.num_workers)

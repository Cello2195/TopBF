...
from core.utils.graph_utils import node_flags, graphs_to_tensor, mask_x
from core.utils.node_features import EigenFeatures
...
def graphs_to_dataloader(config, graph_list, return_feat_dim=False):
    adjs_tensor = graphs_to_tensor(graph_list, config.dataset.max_node_num) 
    x_tensor, feat_dim = init_features(config.dataset, adjs_tensor) 

    dataset = TensorDataset(x_tensor, adjs_tensor)
    dataloader = DataLoader(dataset, batch_size=config.optimization.batch_size, shuffle=True)
    if return_feat_dim:
...
def dataloader(config, get_graph_list=False):
    with open(f'dataset/{config.dataset.name.lower()}.pkl', 'rb') as f:
